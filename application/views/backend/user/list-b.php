<div class="st-content" id="content">
    ...
    <script type="text/javascript">
        function changeStatus(user_id, user_status)
        {
            ...
            jQuery.post("<?php echo base_url(); ?>backend/user-b/change-status", obj_params, function (msg) {
                if (msg.error == "1")
                {
        ...
        function alert_delete(user_id)
        {
            ...
            if (status)
            {
                window.location = '<?php echo base_url(); ?>backend/user-b/delete-user/' + user_id;
                return true;
            }
            else
            {
    ...
    <div class="st-content-inner">
        <div class="container-fluid">
            ...
            <div class="widget-head">
                <a style="float:right;" href="<?php echo base_url(); ?>backend/user-b/add" class="btn btn-plus btn-round" title="Add User B">
                    <button class="btn btn-primary">Add User B</button>
                </a>
            </div>
            <div class="panel panel-default">
                ...
                <form name="frm_users" id="frm_users" action="<?php echo base_url(); ?>backend/user-b/list" method="post">
                    <!-- Data table -->
                    <table id="data-table" class="table" cellspacing="0" width="100%">
                        ...
                        <th width="12%" class="workcap">Institute Name</th>
                        <th width="12%" class="workcap">Village/City</th>
                        <th width="12%" class="workcap">Name Of Founder</th>
                        ...
                        <tbody>
                            ...
                            if (count($arr_user_list) > 0) {
                                ...
                                <td class="worktd"  align="left"><?php echo stripslashes($user['name_of_institute']); ?></td>
                                <td class="worktd"  align="left"><?php echo stripslashes($user['user_city']); ?></td>
                                <td class="worktd"  align="left"><?php echo stripslashes($user['name_of_founder']); ?></td>                                    
                                    ...
                                    if ($user['role_id'] != 1) {
                                        if ($user['user_status'] == 0) {
                                            ...
                                            <div id="blocked_div<?php echo $user['user_id']; ?>" <?php if ($user['user_status'] == 2) { ?> style="display:inline-block" <?php } else { ?> style="display:none;" <?php } ?> >
                                                <a class="label label-warning" title="Click to Change Status" onClick="changeStatus('<?php echo $user['user_id']; ?>', 1);" href="javascript:void(0);" id="status_<?php echo $user['user_id']; ?>">Blocked</a>
                                            </div>
                                            <?php
                                        }
                                        ?>
                                ...
                                <td class="worktd">
                                    <a class="btn btn-info" title="Edit User Details" href="<?php echo base_url(); ?>backend/user-b/edit/<?php echo base64_encode($user['user_id']); ?>">
                                        <i class="icon-edit icon-white"></i>Edit</a>
                                    <a class="btn btn-cyan-A700" title="View User Details" href="<?php echo base_url(); ?>backend/user-b/view/<?php echo base64_encode($user['user_id']); ?>">
                                        <i class="icon-edit icon-white"></i>View</a>
                                    <a class="btn btn-danger" title="Delete User" onclick ="return alert_delete('<?php echo base64_encode($user['user_id']); ?>');"><i class="icon-edit icon-white"></i>Delete</a>
                                </td>
                                <?php
                            }
                        }
