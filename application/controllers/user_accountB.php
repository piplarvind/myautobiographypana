...
class User_AccountB extends CI_Controller {
    ...
    public function userProfile() {
        ...
        $data['header'] = array('title' => 'Institute Timelines');
    ...
    public function userProfile_bkk() {
...
//        echo '<pre>';print_r($data['arr_terms_condtion']);die;
    ...
    public function instituteTimeLine($category_url) {
        $data = $this->common_model->commonFunction();
        ...
        $data['life_event_count'] = count($data['arr_get_time_line']);
        $data['arr_category_list'] = $this->common_model->getMainCategoryList();
    ...
    public function postCommentOnInstituteTimeline() {
        if (!$this->common_model->isLoggedIn()) {
            redirect(base_url());
        }
        $this->load->model("common_model");
        ...
        if ($this->input->post('post_comments') != '') {
            ...
            $str.='<a href = "' . base_url() . 'profile/' . $arr_user_details[0]['user_name'] . '" class = "comment-author">' . ucfirst($data['user_session']['user_name']) . '</a>';
...
//                    $resize_rc = $this->image_lib->resize();
        ...
        $data['arr_get_time_line'] = $this->user_model->getLifeEventCountUserA($this->input->post('category_id'), $this->input->post('sub_category_id'), $data['user_session']['user_id']);
        
        $life_event_count = ($data['arr_get_time_line'][0]['event_count'] + 1);
        ...
        if ($data['arr_user'][0]['user_role'] == '0') {
            $filenames.= '<div><a href="' . base_url() . 'profile/' . $data['arr_user'][0]['user_name'] . '">' . ucfirst(stripslashes($data['arr_user'][0]['first_name'])) . ' ' . ucfirst(substr(stripslashes($data['arr_user'][0]['last_name']), 0, 1)) . '.</a></div>';
        } else {
            $filenames.= '<div><a href="' . base_url() . 'profile/' . $data['arr_user'][0]['user_name'] . '">' . ucfirst(stripslashes($data['arr_user'][0]['user_name'])) . '</a></div>';
        }
        ...
        $filenames.= '<span class="life-event">Life event #'.$life_event_count.'</span>';
        ...
        if ($this->input->post('friend_id') != "") {
            ...
            if ($this->input->post('status') == 'Yes') {// delete record [Unfollow]
                ...
                $insert_data = array(
                    ...
                    'visibility_id' => $this->input->post('visibility_id'),
