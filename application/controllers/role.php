...
class Role extends CI_Controller {
    ...
    public function listRole() {
        ...
        $data['header'] = array('title' => 'Manage Roles');
    ...
    /* To check if user role already exist or not.*/
     public function checkDuplicateUserRole() {
        ...
        $old_role_name = $this->input->post('old_role_name');
        $data['get_role_list'] = $this->user_model->getRoleName($role_name);
        if ($old_role_name != '') {
            if (count($data['get_role_list']) > 0 && ($data['get_role_list'][0]['role_name'] != $old_role_name)) {
                echo "false";
            } else {
                echo "true";
            }
        } else {
            if (count($data['get_role_list']) > 0) {
                echo "false";
            } else {
                echo "true";
            }
    ...
    public function addRole($edit_id = '') {
        ...
        if (count($_POST) > 0) {
            if ($this->input->post('role_name') != "") {
                if ($this->input->post('edit_id') != '') {
                    ...
                    $this->session->set_userdata("msg", "<span class='success'>Role updated successfully, the privileges assigned to role will be updated into admin user account with next login action.</span>");
                } else {
                    /* inserting new role into database */
                    $arr_to_insert = array("role_name" => htmlspecialchars($_POST['role_name']));
        ...
        if (($edit_id != '')) {
            $data['header'] = array('title' => 'Edit Role');
            ...
            $data['title'] = "Add Role";
            $data['header'] = array('title' => 'Add Role');
            $data['edit_id'] = '';
        }
    ...
    public function checkRole() {
        if ($this->input->post('type') == "edit") {
            ...
            if (strtolower($this->input->post('role_name')) == strtolower($this->input->post('old_role_name'))) {
                ...
                $condition_array = array("role_name" => htmlspecialchars($this->input->post('role_name')));
