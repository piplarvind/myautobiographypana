...
$(document).ready(function()
{
    ...
    $.MetroLauncherInit = function (settings) 
        {
            var newContent = "";
            settings = $.extend({
                title: undefined,
		...
		LoadingContent += '<div id="mbCoverUp" class="animated fadeInLeft  green">';
		LoadingContent += '<div align="center">';
		LoadingContent += '<div class="loadingContainer">';
		LoadingContent += '<span id="iconLoading" class="icon-calendar loadingicon"></span>';
		LoadingContent += '<br/><br/>';
        ...
        var SpaceNeeded = $(".mdOptionContainer").length;
        SpaceNeeded = SpaceNeeded * widthNeeded;

        newContent += '<div class="swiper-container swiper-scroll-container size90" style="width: 100% !important;">';
        newContent += '<div class="swiper-wrapper" >';
        newContent += '<div class="swiper-slide" style="width: '+ SpaceNeeded +'px !important;">';

        if ($("#MetroDashboard").length <= 0)
        {
            alert("Where is the Div MetroDashboard?");
        }


        // $("#MetroDashboard").prepend(newContent);

        var DashHTML = $("#MetroDashboard").html();
        ...
        $("#MetroDashboard").html(newContent);
        // $(".swiper-scroll-container").css("width","100%").css("height","100%");
        

        // Setting ID to each Main Div
        $(".mdOptionContainer").each(function(index){
        	$(this).attr("id","mdOption"+index);
        });

        //Setting ID to each small box
        $(".mdOption").each(function(index){
        	$(this).attr("id","mdsmallbox"+index);
        })
        ...
        // });
		$.each( $('.slideshow'), function(i, left) {
			SlideID +=1;

			var Type = $(this).attr("class");
			var SwiperWidth = "";

			if(Type.indexOf("s4x4") >= 0)
			{
				SwiperWidth = "swiper-nested4";
			}
			else
			{
				if(Type.indexOf("s2x2") >=0)
				{
					SwiperWidth = "swiper-nested1";
				}
				else
				{
					SwiperWidth = "swiper-nested2";	
				}
				
			}

			var NewSwiper  = "";
			var NewContent =  '<div class="swiper-slide mbChild">';
				NewContent += '<div class="swiper-container '+ SwiperWidth +' swiper-n'+ SlideID +'">';
				NewContent += '<div class="pagination-n'+ SlideID +'"></div>';
				NewContent += '<div class="swiper-wrapper">'; 
		   ...
		   var swiperN2 = $('.swiper-n'+SlideID).swiper({
			pagination : '.pagination-n'+SlideID,
			slidesPerSlide : 1,
			mode: 'vertical',
			autoPlay: UserTime,
			loop: true
        ...
        $(".mdOption").each(function(index){
        	...
        	if(icon	!= undefined)
        	{
        		icon = "<div align='center' class='iconfijo'><span class='"+ icon +"'></span></div>";
        		$(this).append(icon);
        	}
        	...
        	if(label != undefined)
        	{
        		label = "<div class='labelfijo'>"+ label +"</div>";
        		$(this).prepend(label);
        	}
        	...
        	if(LinkTo != undefined)
        	{
        		$(this).bind("click",function(){
        			var ChangeToIcon = $(this).attr("icon");
        			if(ChangeToIcon== undefined)
        			{
        				ChangeToIcon = "icon-th-large"
        			}
        			var ChangeColor = $(this).attr("class");
        			ChangeColor = ChangeColor.replace("mdOption","").replace("s2x2","").replace("s2x4","").replace("s4x4","").replace("animated fadeIn fast","");

        			$("#iconLoading").removeClass().addClass(ChangeToIcon +" loadingicon");
        			$("#mbCoverUp").removeClass("green").addClass(ChangeColor);
        			$("#mbCoverUp").show().delay(1000).queue(function(){
        ...
        if(WindowHeigth <430)
        {
        	if(WindowHeigth >260)
        	{
        		$(".size90").css("width","99%").css("height","360px");	
        	}
        	else
        	{
        		$(".size90").css("width","99%").css("height","490px");
        	}
...
function AnimateMainDivs()
{
	// alert(AnimateCurrent);
	$("#mdOption"+AnimateCurrent).show().addClass("animated fadeInRight fast").delay(105).queue(function(){
		$(this).clearQueue();
		AnimateCurrent +=1;
		AnimateMainDivs();
	});
...
{
	$("#mdsmallbox"+AnimateSmall).show().addClass("animated fadeIn fast").delay(20).queue(function(){
		$(this).clearQueue();
		AnimateSmall +=1;
		AnimateSmallBox();
	});
